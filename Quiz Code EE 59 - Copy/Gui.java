...
class Gui {
    ...
    GridPane createUI(){
       ...
       // gridPane.setAlignment(Pos.TOP_LEFT);
		...
		gridPane.setStyle("-fx-background-color:#eeeeee; -fx-opacity:100;");
        gridPane.setHgap(20);
        gridPane.setVgap(20);
	...
	void calculatepath1(GridPane gridPane ,String str) {
        // Add Name Label
	...
	void addyourpath(GridPane gridPane,String str){
		        // Add Name Label
        Label nameLabel = new Label("Enter Path : ");
       


        // Path Start Point
        Label xLabel = new Label("Enter Start Node Here ");
        gridPane.add(xLabel, 0, 3);
        TextField xField = new TextField();
        xField.setPromptText("Enter Start Node Here");
        xField.setPrefHeight(30);
        gridPane.add(xField, 1, 3);

        // Add Node Label
        Label yLabel = new Label("Enter Path Here: ");
        gridPane.add(yLabel, 2, 3);
        TextField yField = new TextField();
        yField.setPrefHeight(30);
        yField.setPromptText("Enter Path Nodes here");
        gridPane.add(yField, 3, 3);
		
		Label zLabel = new Label("Enter End Node here: ");
		gridPane.add(zLabel, 4, 3);
        TextField zField = new TextField();
        zField.setPromptText("Enter End Node Here");
        zField.setPrefHeight(30);
        gridPane.add(zField, 4, 3);

        // Add Node Button
        Button addNodeButton = new Button("Check Path");
        addNodeButton.setPrefHeight(40);
        addNodeButton.setDefaultButton(true);
        addNodeButton.setPrefWidth(100);
        gridPane.add(addNodeButton, 0, 4, 4, 2);
        ...
        addNodeButton.setOnAction(event -> {
            if(xField.getText().isEmpty()) {
                showAlert(Alert.AlertType.ERROR, gridPane.getScene().getWindow(), "Error!", "Please enter Start point");
            }else if(yField.getText().isEmpty()) {
                showAlert(Alert.AlertType.ERROR, gridPane.getScene().getWindow(), "Error!", "Please enter at-least 2 nodes");
            
			}else if(zField.getText().isEmpty()) {
                showAlert(Alert.AlertType.ERROR, gridPane.getScene().getWindow(), "Error!", "Please enter End Point");
            }
            else{
               
                try {
                    boolean Pathis = false;
					String output = graph.getPath(xField.getText(), zField.getText());
                    
					String yourpath = yField.getText();
                    String[] splitpath = yourpath.split(",");
					String[] splitoutput = output.split("->");
					for (int i = 1; i <=splitpath.length-1; i++)
					{
						if (splitpath[i]==splitoutput[i+1])
						{
							
							Pathis = false;
							//System.out.println(splitpath[i]+" = "+ splitoutput[i+1]);
					 ...
					 if (Pathis!= true)
					 {showAlert(Alert.AlertType.INFORMATION, gridPane.getScene().getWindow(), " Your path Incorrect. Correct Path is:  ",output);
					 }
					 else
					 {
						 showAlert(Alert.AlertType.INFORMATION, gridPane.getScene().getWindow(), "Your path Correct Path ", output);
					 }
                ...
                if(str.equals("Add Node")) {
                    String output = "Path Entered";
                    showAlert(Alert.AlertType.CONFIRMATION, gridPane.getScene().getWindow(), "Addition Successful!", output);
                }
                else if(str.equals("Modify Node")) {
					String output = "Path Changed";
                    //String output = graph.ModifyNode("Path Entered",x,y);
                    showAlert(Alert.AlertType.CONFIRMATION, gridPane.getScene().getWindow(), "Modification Successful!", output);
                }
